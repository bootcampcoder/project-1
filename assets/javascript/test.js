    ...
    subCategories.forEach(element => {
      debugger;
        ...
        $("#sel-sub-category").append(
          `<option class="sub-category-items" data-id=${element.id}>${element.name}</option>`
        );
      }
    });
}
  ...
  var settings1 = {
    ...
    $.ajax(settings1).done(function(data1) {
      ...
      categories.forEach(element => {
        $("#sel-category").append(`<option class="category-items" data-id=${element.id}>${element.name}</option>`);
